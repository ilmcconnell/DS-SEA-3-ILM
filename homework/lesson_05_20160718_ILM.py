...
with open('/Users/Iain/DS-SEA-3/DS-SEA-3-ILM/homework/example.html', 'rU') as f:
    html = f.read()
...
for chunk in b.find_all(name='li'):
    print chunk.text

# print the text of only the API resources
for chunk in b.find(name='ul', attrs={'id':'api'}).find_all(name='li'):
    print chunk.text
'''
...
f = codecs.open(r'/Users/Iain/DS-SEA-3/DS-SEA-3-ILM/homework/Shawshank.html', "w", 'utf-8')
f.write (b.prettify()) 
f.close()
...
b.find(name='h1', attrs={'class':'', 'itemprop':'name'}).text.replace(u'\xa0', u' ').lstrip().rstrip()

import re as regex

regex.sub('\([0-9]+\)', '', (b.find(name='h1', attrs={'class':'', 'itemprop':'name'}).text)).replace(u'\xa0', u' ').lstrip().rstrip()
...
# get the description <div class="summary_text" itemprop="description">
b.find(name='div', attrs={'class':'summary_text','itemprop':'description'}).text.lstrip().rstrip()
...
int(b.find_all(name='time', attrs={'datetime':'PT142M','itemprop':'duration'})[1].text[0:3])
int(b.find(name='time', attrs={'itemprop':'duration'})['datetime'].replace('PT','').replace('M',''))
...
def getMovie(IMDbID):
    ...
    b = BeautifulSoup(r.text)
    resultDictionary['Content_rating'] = b.find(name='meta', attrs={'itemprop':'contentRating'})['content']
    resultDictionary['Description'] = b.find(name='div', attrs={'class':'summary_text','itemprop':'description'}).text.lstrip().rstrip()
    resultDictionary['Duration'] =  int(b.find(name='time', attrs={'itemprop':'duration'})['datetime'].replace('PT','').replace('M','')) 
    resultDictionary['star_rating'] = float(b.find(name='span', attrs={'itemprop':'ratingValue'}).text)
    resultDictionary['title'] = regex.sub('\([0-9]+\)', '', (b.find(name='h1', attrs={'class':'', 'itemprop':'name'}).text)).replace(u'\xa0', u' ').lstrip().rstrip()
    return resultDictionary

# test the function

print getMovie('tt0111161')

 
# open the file of IDs (one ID per row), and store the IDs in a list
path = '/Users/Iain/DS-SEA-3/DS-SEA-3/data/imdb_ids.txt'
IdList = open(path).readlines()

scrubbedList = []
for i in range(0, len(IdList)):
    scrubbedList.append(IdList[i].rstrip())
